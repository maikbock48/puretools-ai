  ...
  return (
    <Link href={href}>
      <motion.div
        ...
        className={`group relative overflow-hidden rounded-2xl border bg-zinc-900 p-6 transition-all duration-300 ${
          isLocal
            ? 'border-emerald-500/20 hover:border-emerald-500/40 hover:glow-emerald'
            : 'border-indigo-500/20 hover:border-indigo-500/40 hover:glow-indigo'
        }`}
      >
        {/* Background gradient */}
        <div
        ...
        >
          <svg
            className={`h-6 w-6 ${isLocal ? 'text-emerald-400' : 'text-indigo-400'}`}
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
          >
